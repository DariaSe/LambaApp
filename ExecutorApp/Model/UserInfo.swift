...
struct UserInfo {
    ...
    static func initialize(from dictionary: Dictionary<String, Any>, completion: @escaping ((UserInfo?) -> Void)) {
        ...
        socialsDispatchGroup.notify(queue: .main) {
            let userInfo = UserInfo(socialMedia: socialMediaArr, hashtags: hashTags, orderSettings: orderSettings, isReceivingOrders: isReceivingOrders, currencySign: currencySign, role: userRole)
            completion(userInfo)
        }
    }
